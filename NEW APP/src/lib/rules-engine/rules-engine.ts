...
import { isMowableForageCrop, isPreparedSeedbedGround } from "./forage-crop-types";
import { isCoverCrop } from "./cover-crop-types";
...
function clearPhantomCropOnPreparedGround(field) {
  if (!field || !isPreparedSeedbedGround(field.groundType, field.groundTypeName)) return field;
  const fruit = (field.fruitType || "").toLowerCase();
  const keepMulch =
...
function isSoilTilledField(field) {
  field = clearPhantomCropOnPreparedGround(field);
