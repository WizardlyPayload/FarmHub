/**
 * Compare local app/mod sources to WizardlyPayload/FS25-Farm-Dashboard main (raw).
 * Run from repo root: node tools/diff-with-upstream.mjs
 * Exit 1 if any file differs or fails to fetch.
 */
...
const FILES = [
  ...
  ["FS25_Dashboard MOD/src/FarmDashboard.lua", path.join(MOD, "src", "FarmDashboard.lua")],
  [
...
async function main() {
  ...
  for (const [remote, dest] of FILES) {
    ...
    const name = path.relative(WORKSPACE, dest);
    try {
      ...
      if (localText !== remoteText) {
        console.log("DIFF", name, `(local ${localText.length} vs remote ${remoteText.length} chars)`);
        diff++;
      } else {
        console.log("OK  ", name);
      }
  ...
  console.log(`\nSummary: ${diff} differ/missing/fail, ${FILES.length - diff} match.`);
