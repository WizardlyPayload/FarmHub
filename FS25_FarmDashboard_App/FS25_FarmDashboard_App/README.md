...
**Version 4.0.0** — Real-time farm management dashboard for *Farming Simulator 25*, with local save folders and dedicated (FTP) servers.
...
1. Run **`FS25 Farm Dashboard Setup 4.0.0.exe`** (or the current release build from `npm run dist`).
...
`%LOCALAPPDATA%\fs25-farm-dashboard-electron-out\` — e.g. `FS25 Farm Dashboard Setup 4.0.0.exe`, `latest.yml`, and `win-unpacked/` (version follows `package.json`). See **[`tools/README.md`](../../../tools/README.md)**.
