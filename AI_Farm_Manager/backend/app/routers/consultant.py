...
async def get_consultant_insights(_: str = Depends(require_integration_or_admin)) -> FarmInsightsResponse:
    """
    Fetch current dashboard snapshot (FTP in-memory or DASHBOARD_JSON_URL), run heuristics + LLM analysis.
    ...
    insights, llm_used = await generate_farm_insights(snapshot)
