...
**App v3.1.0 (current):** Servers/saves configuration is consolidated in **Settings → Servers & saves** (`openUnifiedSettingsModal` in `dashboard-settings.js`); the duplicate navbar folder shortcut was removed. This release adds **local / OpenAI-compatible** LLM support (Ollama, LAN) — see [CHANGELOG.md](./CHANGELOG.md) §3.1.0; the **3.0.0** baseline (Settings UX, LAN, BYOK polish, build output) is in [RELEASE_v3.0.0.md](./RELEASE_v3.0.0.md).

**Deep dive (Gemini):** [LLM_GEMINI_ROUTING.md](./LLM_GEMINI_ROUTING.md) · **OpenAI-compatible / Ollama / LAN:** [LLM_OPENAI_COMPATIBLE.md](./LLM_OPENAI_COMPATIBLE.md) · **AI API hardening (public deploys):** [AI_SERVER_SECURITY.md](./AI_SERVER_SECURITY.md)
...
| [4](#4-ai-farm-manager-backend)                     | AI Farm Manager backend, routers, consultant, **LLM** (Gemini + OpenAI + **OpenAI-compatible**) |
| [5](#5-cross-system-data-flow-consultant)           | Consultant data flow                                  |
| [5.1](#51-farm-dashboard-express-proxy-byok--tiers) | Express proxy, **BYOK on-device**, `suggestion_tier`  |
...
flowchart LR
  ...
  subgraph vps [VPS / Coolify / TrueNAS]
  ...
  AI -->|Gemini / OpenAI / OpenAI-compatible LAN| LLM[LLM APIs]
```
...
### 1.1 Phase 2 Electron security (renderer)

- `**nodeIntegration: false`** and `**contextIsolation: true`** in `BrowserWindow` `**webPreferences**` (`main.js`).
- `**preload.js**` runs in an isolated world and exposes `**window.farmDashAPI**`: whitelisted `**ipcRenderer.invoke` / `send**` channels only (config, locale, LAN settings, mod export progress, AI/BYOK, etc.). **No** raw `ipcRenderer` and **no** Node `**fs`** / `**path`** in the page — legacy `require('electron')` in `web/**/*.js` was removed.
- **Setup** (`setup.html`) and the main SPA use the same bridge for first-run and ongoing IPC.
...
**Splash / first paint (`splash-screen.js`):** `index.html` shows `**#splash-screen`** until the first successful `**/api/data`** merge (not AI). The script enforces a **minimum visible duration** so the bar does not vanish instantly on fast machines. Smart suggestions may still **eager-load** cached insight payloads in parallel — see logs in `ai-farm-consultant-insights.js`.
...
| Router                | Prefix               | Notes                                                                                                                                                              |
| --------------------- | -------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `chat`                | `/api/chat/...`      | In-game **Hank** (`!hank`): **multiplayer only** (host/dedicated/rented server). Not used in single-player career — see [AI_IN_GAME_CHAT.md](./AI_IN_GAME_CHAT.md) |
...
5. **LLM** — `llm_service.gemini_consultant_post_with_quota_fallback` (Gemini) or OpenAI path when `LLM_PROVIDER=openai` (cloud or **OpenAI-compatible** via `OPENAI_BASE_URL` — see §4.6).

### 4.4 LLM / Gemini (`app/services/llm_service.py`)

**Authoritative detail:** [LLM_GEMINI_ROUTING.md](./LLM_GEMINI_ROUTING.md) — key order, model stack, 429/503, BYOK, and how this differs from `active_gemini_api_key()` in `config.py`.
...
**Concurrent UI bursts:** Multiple dashboard tabs or rapid navigation can trigger overlapping consultant requests. **Strict round-robin** spreads the *first* key choice across those concurrent calls; **top-down model degradation** (rollover list, best model first on each new request) handles **429/503** without a single long-lived “stuck on worst model” state. Client-side guards (`insightsFetchInFlight`, debounces) still matter so the UI does not stack duplicate work.
...
| `OPENAI_BASE_URL`                             | Optional **OpenAI-compatible** API base (Ollama/LAN); empty = OpenAI cloud ([LLM_OPENAI_COMPATIBLE.md](./LLM_OPENAI_COMPATIBLE.md)) |
| `LLM_API_KEY`                                 | OpenAI key **or** placeholder (`ollama`) when using local compatible servers without auth                         |
...
| `REQUIRE_AUTH_FOR_ROOT_HTML`                  | When `1`, `**GET /**` requires integration key or admin Basic ([AI_SERVER_SECURITY.md](./AI_SERVER_SECURITY.md)) |
| `HEALTH_RESPONSE_DETAIL`                      | `full` (default) or `minimal` for `**/health**` JSON ([AI_SERVER_SECURITY.md](./AI_SERVER_SECURITY.md))          |
| `CORS_ORIGINS`                                | Comma origins or `*`; `*` implies no credentialed CORS ([AI_SERVER_SECURITY.md](./AI_SERVER_SECURITY.md))        |
...
### 4.6 OpenAI-compatible / local LLM (Ollama, vLLM, LAN)

When `LLM_PROVIDER=openai` and **`OPENAI_BASE_URL`** is set (or BYOK sends **`X-AI-OpenAI-Base-URL`**), the backend uses the **OpenAI Python SDK** with `base_url` pointing at an **OpenAI-compatible** server. This is **not** Gemini routing; see **[LLM_OPENAI_COMPATIBLE.md](./LLM_OPENAI_COMPATIBLE.md)** for env vars, headers, and the **file manifest**.

Summary:

- **`normalize_openai_base_url_for_sdk()`** in `config.py` ensures the SDK sees a `…/v1` base path when omitted.
- **`async_openai_client()`** / **`effective_openai_api_key()`** in `llm_service.py` — placeholder **`ollama`** when only a base URL is configured.
- **Consultant** (`consultant.py`) — BYOK merges clear the server’s `openai_base_url` unless the request supplies **`X-AI-OpenAI-Base-URL`**.
- **Farm Dashboard** — provider **`openai_compat`** in Electron store; proxy sends **`X-AI-Provider: openai`** plus optional **`X-AI-OpenAI-Base-URL`**.
...
### 5.1 Farm Dashboard Express proxy, BYOK on-device, and tiers

The **renderer never calls the VPS URL directly.** The browser hits the embedded Express app:

- `**GET /api/farmdash-ai/consultant/insights`** (`main.js`) — single entry for Smart suggestions.

**Decision order (simplified):**

1. **BYOK + localhost client:** If `getConsultantByokCredentialPairs()` is non-empty and the request is from the **local** dashboard (`isLocalDashboardClient`), the main process builds a merged snapshot, `**pruneMergedDataForView`** (`consultantSnapshotPrune.js`), **SHA-256 hashes** the pruned JSON per lane (`serverId|farmId|view|context`), and on a **hash hit** returns cached JSON without calling the LLM. On miss, `**runByokConsultantLlm`** (`localConsultantLlm.js`) calls OpenAI/Gemini **on this PC**, or an **OpenAI-compatible** URL when BYOK **Local / OpenAI-compatible** + base URL are saved. Response includes `farmdash_byok_local: true`, `suggestion_tier: 'byok'`, `llm_used: true` on success.
2. **BYOK + non-localhost:** Returns **503** / LAN cache miss — LLM runs only on the host PC; tablets reuse **consultantInsightsProxyCache** when populated.
3. **Hosted AI:** If URL + integration key are configured, **fetch** to `{baseUrl}/api/v1/consultant/insights` with `X-FarmDash-Key` (+ optional `X-AI-API-Key`, `X-AI-Provider`, **`X-AI-OpenAI-Base-URL`**). JSON is merged so `**suggestion_tier`** is `hosted` if `llm_used`, else `rules`, unless the upstream body already sets it.

**UI:** `ai-farm-consultant-insights.js` maps tiers to badges (**Hosted** / **BYOK** / **Rules**). `field-consultant-bridge.js` exposes `**deriveSuggestionTier`** and `window.__fieldConsultantSuggestionTier` for the Fields tab.

**Authoritative one-pager:** [SMART_SUGGESTIONS_TIERS.md](./SMART_SUGGESTIONS_TIERS.md).
...
- **Public / VPS API hardening (optional):** See [AI_SERVER_SECURITY.md](./AI_SERVER_SECURITY.md). Key env vars: `**REQUIRE_AUTH_FOR_ROOT_HTML`** (protects `**GET /`** snapshot HTML), `**HEALTH_RESPONSE_DETAIL=minimal**` (slim `**/health**`), `**CORS_ORIGINS**` (explicit origins if browsers need credentialed CORS; `*` disables credentials by design).
...
| **Backend**           | **Gemini routing** — multi-key: **round-robin** start index per request; **429/503:** step through `**GEMINI_MODEL_ROLLOVER`** (best→fallback) on each key before rotating keys; BYOK uses one key + same model stack ([LLM_GEMINI_ROUTING.md](./LLM_GEMINI_ROUTING.md)).                                                                                                                                                                  |
| **Backend**           | **Public API hardening** — optional `**REQUIRE_AUTH_FOR_ROOT_HTML`**, `**HEALTH_RESPONSE_DETAIL`**, spec-safe `**CORS_ORIGINS=***` ([AI_SERVER_SECURITY.md](./AI_SERVER_SECURITY.md)).                                                                                                                                                                                                                                                     |
| **Backend**           | **OpenAI-compatible** — `**OPENAI_BASE_URL**`, `**async_openai_client()`** in `llm_service.py`; consultant resolve + **`X-AI-OpenAI-Base-URL`** ([LLM_OPENAI_COMPATIBLE.md](./LLM_OPENAI_COMPATIBLE.md)).                                                                                                                                                                                                                                  |
| **Frontend (app)**    | **BYOK Local / OpenAI-compatible** — `index.html` provider + base URL; `main.js` / `ai-farm-bot-panel.js` / `localConsultantLlm.js`.                                                                                                                                                                                                                                                                                                      |
...
End-user BYOK setup: [AI_FARM_MANAGER_BYOK.md](./AI_FARM_MANAGER_BYOK.md).
...
| ---------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 429 / rate limits            | Server logs (`Gemini HTTP 429`), `gemini_budget.py`, add keys or shorten `**GEMINI_MODEL_ROLLOVER`** (drop heavy preview models). See [LLM_GEMINI_ROUTING.md](./LLM_GEMINI_ROUTING.md). |
...
| `FS25_FarmDashboard_App/.../main.js`                         | Express routes incl. `**/api/farmdash-ai/consultant/insights**`, BYOK + hosted branches, IPC, LAN auth middleware |
| `FS25_FarmDashboard_App/.../preload.js`                      | `**contextBridge**` — `**window.farmDashAPI**` IPC surface for the renderer                                       |
| `FS25_FarmDashboard_App/.../consultantSnapshotPrune.js`      | Per-`view` snapshot pruning + hash for BYOK cache                                                                 |
| `FS25_FarmDashboard_App/.../localConsultantLlm.js`           | On-device OpenAI/Gemini/**OpenAI-compatible** for BYOK                                                            |
| `FS25_FarmDashboard_App/.../ai-farm-consultant-insights.js`  | Smart panel client                                                                                                |
| `FS25_FarmDashboard_App/.../field-consultant-bridge.js`      | Per-field consultant map, `**deriveSuggestionTier**`                                                              |
...
*Maintenance: update when you add routes, env vars, deployment steps, or change LLM routing — [LLM_GEMINI_ROUTING.md](./LLM_GEMINI_ROUTING.md) (Gemini), [LLM_OPENAI_COMPATIBLE.md](./LLM_OPENAI_COMPATIBLE.md) (OpenAI cloud + compatible / Ollama).*