...
The dev handover documents both flows, the order to run them in, and lists the seven outstanding language line-packs as a tracked follow-up. The user manual sticks to the user-visible **language picker** behaviour and does not mention the build pipeline. **(Historical:** current **[DEVELOPER_HANDOVER.md](./DEVELOPER_HANDOVER.md)** / **[_internal/I18N.md](./_internal/I18N.md)** describe **`messages/*.json` only** — segment artifacts were removed from the repo.)
...
| 5 | Seven `line-packs/<lang>.txt` files outstanding | **Obsolete** — segment pipeline removed; use **`messages/*.json`** + **`npm run i18n:sync`** per **[_internal/I18N.md](./_internal/I18N.md)** |
...
- [`SCREENSHOTS.md`](./SCREENSHOTS.md) — image filenames, captions, `[auto]` / `[manual]` capture method, and exact recipe per shot.
