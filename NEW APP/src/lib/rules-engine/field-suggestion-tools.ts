...
import { RULES_ENGINE_FALLBACK_ACTION } from "./rules-engine";
import { getBaseGameLabelsForRole } from "./base-game-tool-catalog";
 ...
 */
/**
 * Fleet matches for field suggestions — used for Vehicles deep links (`role` hash param).
 * @returns {{ roleId: string, roleLabel: string, vehicleName: string }[]}
 */
export function buildFleetLinksForSuggestion(vehicles, farmId, action, field, actionKey) {
  const roleIds = inferToolRoleIds(field || {}, action, actionKey);
  if (!roleIds.length || !action) return [];
  const { owned } = matchToolRolesToFleet(vehicles, farmId, roleIds, field, actionKey);
  return owned.map(({ roleId, matches }) => {
    const meta = roleById(roleId);
    return {
      roleId,
      roleLabel: meta ? t(meta.labelKey) : roleId,
      vehicleName: matches?.[0] ? String(matches[0]) : "",
    };
  });
}
